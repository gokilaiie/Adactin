...
public class DfltAdult_SrchHotel 
{
	@Test
	public void main()
	{
		try
		{
			...
			driver.get("https://adactinhotelapp.com/SearchHotel.php");
			 ...
			 AdRoom.selectByVisibleText("- Select Adults per Room -");
			 ...
			 WebElement Error = driver.findElement(By.xpath("//span[@id='adults_room_span']"));
				if(Error.isDisplayed())
				{
					String Serror = Error.getText();
					System.out.println();
					assertEquals(Serror, "Please Select Adults per Room");
								
				}
				else
				{
					assertEquals(Error, "Please Select Adults per Room");
				}
